  ...
  Widget buildAddRowButton() {
    return Padding(
      ...
      child: ElevatedButton(
        onPressed: () {
          bool reasonNotEmpty = true;
          for (TextEditingController controller in secondTypeAheadControllers) {
            if (controller.text.isEmpty) {
              reasonNotEmpty = false;
              break;
            }
          }
          if (reasonNotEmpty) {
            ...
            // Show a message indicating that the reason must not be empty
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text('Please provide a reason for the current row before adding a new one.'),
      ...
      child: ElevatedButton(
        onPressed: () async {
          ...
          if (isFormValid() && allFieldsFilled && globalnetBalance! >= amount!) {
            ...
            } else if (globalnetBalance! < amount!) {
              errorMessage = 'Please Enter Quantity Lower Than The Current Balance.';
            } else {
              errorMessage = 'Invalid form. Please check your inputs.';
            }
            print(errorMessage);
